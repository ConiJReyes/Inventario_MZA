...
def movimientos(request):
    ...
    toast_alerta = None
    productos_stock_bajo = None  # solo lo usamos si hay POST
    ...
    if request.method == 'POST':
        ...
        # Alerta interna: salida mayor a 50 unidades
        if tipo_movimiento == 'salida' and cantidad > 50:
            mensaje = f"Se ha realizado un movimiento de salida de más de 50 unidades del producto {producto.nombre}."
        ...
        # Alerta interna: stock bajo
        if producto.stock < STOCK_UMBRAL_BAJO:
            mensaje_stock_bajo = f"El stock de {producto.nombre} es bajo. Solo queda {producto.stock} unidades."
            Notificacion.objects.create(usuario=request.user, mensaje=mensaje_stock_bajo)
            productos_stock_bajo = Producto.objects.filter(stock__lt=STOCK_UMBRAL_BAJO, estado=True)

        if tipo_elemento == 'pieza':
    ...
    # Si es GET, no mostrar alertas ni notificaciones
    productos = Producto.objects.filter(estado=True)
    movimientos = Movimiento.objects.all().order_by('-fecha')
    return render(request, 'inventario_app/movimientos.html', {
        'movimientos': movimientos,
...
#HTML stock bajo
@login_required
def productos_stock_bajo_view(request):
    productos = Producto.objects.filter(stock__lt=STOCK_UMBRAL_BAJO, estado=True).order_by('nombre')
    return render(request, 'inventario_app/stock_bajo.html', {'productos': productos})
