        ...
        # ----- FLUJO PARA PIEZAS Y KITS -----
            ...
            if tipo_movimiento == 'salida':
                ...
                if producto.stock < cantidad:
                    return render(request, 'inventario_app/movimientos.html', {
                        'error': f'No hay suficiente stock disponible. Stock actual: {producto.stock}, solicitado: {cantidad}.',
                        'productos': Producto.objects.filter(estado=True),
                        'kits': Kit.objects.all(),
                        'lotes': Lote.objects.filter(estado='activo'),
                        'movimientos': Movimiento.objects.all().order_by('-fecha'),
                        'movimientos_count': Movimiento.objects.count(),
                        'toast_alerta': f"¡Alerta! No hay suficiente stock para la salida de {producto.nombre}. Stock actual: {producto.stock}.",
                    })

                # Salida válida: descontamos
                producto.stock -= cantidad
                if producto.stock == 0:
                    producto.estado = False
                    producto_agotado = producto.nombre

                # Notificación por salida grande SOLO si sí se realizó
                if cantidad > 50:
            ...
            # Validación de stock bajo luego de actualizar stock
            if producto.stock < STOCK_UMBRAL_BAJO:
                mensaje_stock = f"El stock de {producto.nombre} es bajo. Quedan {producto.stock} unidades."
                ...
                toast_alerta = f"¡Alerta! El stock de {producto.nombre} es bajo. Solo quedan {producto.stock} unidades."
